...
class MyConfiguration(object):
  def __init__(self, argv):
    ...
    for index, r in enumerate(self.CACHE_IGNORE_TYPES):
      self.CACHE_IGNORE_TYPES[index] = re.compile(re.sub(r"^\^image://", r"^", r.pattern))
    for index, r in enumerate(self.PRUNE_RETAIN_TYPES):
      self.PRUNE_RETAIN_TYPES[index] = re.compile(re.sub(r"^\^image://", r"^", r.pattern))
      ...
      try:
        ...
        if "movieid" in item:
          ...
          if year and title.endswith("(%d)" % year):
            title = re.sub(r"\(%d\)$" % year, r"", title).strip()
          isTVShow = True
...
class MyJSONComms(object):
  ...
  def logreplay_mapthread(self, map_to_thread=None):
    ...
    self.web_re_result = re.compile(r"^.*:(%s)[ ]*: .*\.RECEIVED WEB DATA: ([0-9]*), (.*), (.*)$".format(tpattern))
    self.json_re_result = re.compile(r"^.*:(%s)[ ]*: .*\.PARSING JSON DATA: (.*)$".format(tpattern))
    ...
    for tvshow in data:
      ...
      if not ignoreShow:
        ...
        if tvshow["tc.year"] is not None and tvshow["title"].endswith("(%d)" % tvshow["tc.year"]):
          before_title = tvshow["title"]
          tvshow["title"] = re.sub(r"\(%d\)$" % tvshow["tc.year"], r"", tvshow["title"]).strip()
